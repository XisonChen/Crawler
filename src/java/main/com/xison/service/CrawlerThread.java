...
public class CrawlerThread implements Runnable {
    ...
    private List<WebPage> parsePlaylists(WebPage webPage) {
        ...
        return playlists.stream().map(e -> new WebPage(BASE_URL + e.attr("href"), WebPage.PageType.playlist))
            .collect(Collectors.toList());
    }

    private List<WebPage> parsePlaylist(WebPage webPage) {
        Elements songs = Jsoup.parse(webPage.getHtml()).select("ul.f-hide li a");
        return songs.stream().map(e -> new WebPage(BASE_URL + e.attr("href"), WebPage.PageType.song, e.html()))
            .collect(Collectors.toList());
    }
